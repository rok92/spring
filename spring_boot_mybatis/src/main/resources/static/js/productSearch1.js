 	...
 	$('#prdSearchFrm').on('submit', function(){
 		...
 		$.ajax({
 			...
 			success: function(result){ // 컨트롤러에서 반환한 prdList를 result가 받음
 				...
 				'<tr><th>상품번호</th><th>상품명</th><th>상품가격</th><th>제조회사</th><th>재고</th><th>제조일</th><th>사진</th></tr>');
 				
 				if(result == "") {
 					$('#resultTable').append('<tr align="center"><td colspan="6">찾는 상품이 없습니다.</td></tr>');
 				} else {
 					for(var i=0; i<result.length; i++) {
 						$('#resultTable').append('<tr><td>' + result[i].prdNo + '</td><td>' + result[i].prdName +
 						'</td><td>' + result[i].prdPrice + '</td><td>' + result[i].prdCompany +
 						'</td><td>' + result[i].prdStock + '</td><td>' + new Date(result[i].prdDate).toLocaleDateString('en-CA') +
 						'</td><td>' + 
